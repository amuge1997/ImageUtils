...
def resize_image(im, size_w_h):
    # im.shape = h,w,3
    # im.dtype = uint8
    im = cv.resize(im, size_w_h)
    return im
...
class Video:
    ...
    def read_video_list_by_generate(self):
        self.cap.set(cv.CAP_PROP_POS_FRAMES, 0)
        for i in range(int(self.max_index)):
            flag, frame = self.cap.read()
            if not flag:
                break
...
class MP4Maker:
    def __init__(self, save_path, fps, h, w):
        # print()
        fourcc = cv.VideoWriter_fourcc(*"mp4v")
        self.video = cv.VideoWriter(save_path, fourcc, fps, (w, h))
...
def make_mp4_from_dir(im_dir, save_path, fps, rotate=0, select_index_callback=None, is_use_generator=False):
    import os
    print()
    print('图像以整数数字作为序号!')
    files = os.listdir(im_dir)
    ...
    im_shape = read_image(im_dir + '/' + files[0]).shape

    if is_use_generator:
        maker = MP4Maker(save_path, fps, im_shape[0], im_shape[1])
...
def sample_by_probability(lis, nums, prob=None):
    # lis = [obj, obj, ...]

    nel = len(lis)
    if prob is None:
        prob = [1 / nel for _ in range(nel)]

    if len(prob) != nel:
        raise Exception

    if abs(sum(prob) - 1.) > 1e-2:
        raise Exception

    prob_array = n.zeros((nel,))
    prob_array[0] = prob[0]
    for i in range(1, nel):
        prob_array[i] = prob_array[i-1] + prob[i]

    ret = []
    for i in n.random.rand(nums):
        ix = n.where(i < prob_array)[0][0]
        ret.append(lis[ix])
    return ret


def stack_image_nxn_auto(im):
    # im.shape = m, h, w, 3 or m, h, w, 1 or m, h, w
    ...
    ne = n.zeros((p*h, p*w, c), dtype=im.dtype)
    for i in range(p):
        for j in range(p):
            ne[i*h:(i+1)*h, j*w:(j+1)*w, :] = im[int(i*p+j), ...]
